...
<div>
    @if ($paginator->hasPages())
        <nav role="navigation" aria-label="Pagination Navigation" class="flex items-center justify-around">
            <div class="flex justify-between items-center w-full mx-2 sm:hidden">
                <span>
                    @if ($paginator->onFirstPage())
                        <span class="relative items-center inline-flex px-2 py-2 text-sm font-medium vinyler-text-color bg-red-50 custom-shadow border-4 border-slate-300 cursor-default leading-5 select-none">
                            Föregående
                        </span>
                    @else
                        <button type="button" wire:click="previousPage('{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" wire:loading.attr="disabled" dusk="previousPage{{ $paginator->getPageName() == 'page' ? '' : '.' . $paginator->getPageName() }}.before" class="relative items-center px-2 py-2 text-sm font-medium vinyler-text-color bg-white custom-shadow border-4 border-slate-300 leading-5 hover:text-gray-500 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150" aria-label="Next Page">
                            Föregående
                        </button>
                    @endif
                </span>

                <span class="relative inline-flex text-sm items-center bg-white px-2 py-2 ml-5 border-4 custom-shadow border-slate-300">Sida&nbsp;&nbsp;<span class="font-semibold">{{ $paginator->currentPage() }}</span>&nbsp;&nbsp;/&nbsp;&nbsp;{{ $paginator->lastPage() }}</span>

                <span>
                    @if ($paginator->hasMorePages())
                        <button type="button" wire:click="nextPage('{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" wire:loading.attr="disabled" dusk="nextPage{{ $paginator->getPageName() == 'page' ? '' : '.' . $paginator->getPageName() }}.before" class="relative inline-flex items-center px-2 py-2 ml-5 text-sm font-medium vinyler-text-color bg-white  custom-shadow border-4 border-slate-300 leading-5 hover:text-gray-500 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150" aria-label="Previous Page">
                            Nästa
                        </button>
                    @else
                        <span class="relative inline-flex items-center px-2 py-2 ml-5 text-sm font-medium vinyler-text-color bg-red-50  custom-shadow border-4 border-slate-300 cursor-default leading-5 select-none">
                            Nästa
                        </span>
                    @endif
                </span>
            ...
            <div class="hidden pl-0 sm:flex-1 sm:flex sm:items-center sm:justify-between">
                {{-- <div>
                ...
                <div class="custom-shadow rounded-b-lg">
