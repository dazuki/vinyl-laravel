<div wire:init="init" class="max-w-screen-xl mx-auto text-left">
                    ...
                    <tbody>
                        <tr class="bg-white border-slate-300 hover:bg-slate-50">
                            <td colspan="2" class="text-center bg-white">
                                <div wire:loading
                                    class="inline-block px-2 py-4 text-base text-gray-900 align-top sm:px-6 sm:text-lg">
                            ...
                            @if ($artists->count() >= 1)
                                @foreach ($artists as $artist)
                                    ...
                                    <tr wire:key="artist-{{ $artist->id }}"
                                        class="bg-white border-b border-slate-300">
                                        <td
                                            class="px-2 py-2 pb-0 text-lg font-bold text-gray-700 align-top rock-font sm:px-6 sm:text-xl lg:text-3xl">
                                            <a href="/artist/{{ $artist->id }}"
                                                class="antialiased hover:text-blue-800" wire:navigate>
                                                @if (!empty($search))
                                                    @php
                                                        ...
                                                        @if (!empty($search))
                                                            @php
                                                                ...
                                                                echo "<a href='https://www.discogs.com/search/?q=" .
                                                                    urlencode(
                                                                        $artist->name . ' ' . $record->record_name,
                                                                    ) .
                            ...
                            @else
                                @if ($artists->count() <= 0)
                                    <tr class="bg-white border-b-2 border-slate-300 lg:border-l-2 lg:border-r-2">
                                        <td colspan="2"
                                            class="px-2 py-4 text-base text-center text-gray-900 align-top sm:px-6 sm:text-lg">
                                            <p>Sökord gav inga träffar...</p>
                                            @auth
                                                <div class="flex items-center justify-center mt-4">
                                                    <a class="p-2 font-semibold text-center text-gray-900 border-2 rounded-lg shadow-md border-slate-300 bg-slate-100 hover:text-green-700 max-sm:w-full"
    ...
    @else
        <div
            class="flex justify-center w-full min-h-screen px-2 py-4 text-base text-gray-900 align-top bg-white items-top border-slate-300 hover:bg-slate-50 sm:px-6 sm:text-lg">
